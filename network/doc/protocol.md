...
> 如果发生因信息传输导致的双方共同发送请求，即检查对方的第二字段，发现非空，说明出现了这个情况。为了统一，拓展协议中规定此时听服务端的，即当做客户端没发过 `READY_OP`，双方无视掉这条通信，所以此时客户端应该先回应是正常回复 `READY_OP` 并置第二字段为空，还是拒绝 `REJECT_OP`。这种情况出现的概率应该极小，如果出现了，大概率是没有按照 **如果已经收到对方想再来一局了，程序应该阻止本方重复发送请求，而引导先接受或拒绝** 来实现。
